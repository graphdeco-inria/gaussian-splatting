"""Small wrappers for running the validated project containers."""
...
class Runtime:
    ...
    def _container_path(self, value):
        ...
        # Leave relative arguments and image names unchanged.
        path = Path(value)
        if not path.is_absolute():
            return value
        try:
            # Repository files are mounted read-only at ``/repo``.
            return "/repo/" + path.relative_to(self.repo_root).as_posix()
        except ValueError:
            pass
        try:
            # Dataset and output files are mounted read-write at ``/data``.
    ...
    def _docker_command(self, image, gpu, command):
        ...
        ``gpu`` is a boolean flag that adds the NVIDIA runtime when enabled.
    ...
    def _run(self, command):
        ...
        # Print the exact command so a failed stage can be reproduced manually.
    ...
    def run_fusion(self, script, arguments):
        ...
        command = self._docker_command(
            self.fusion_image, True, ["python", script] + args,
        )
        self._run(command)
    ...
    def run_fusion_module(self, module, arguments):
        ...
        args = [self._container_path(str(item)) for item in arguments]
        command = self._docker_command(
            self.fusion_image, True, ["python", "-m", module] + args,
        )
        self._run(command)

    def run_train(self, dataset_dir, model_dir, iterations, resolution,
                  data_device="cuda"):
        """Run Gaussian training with the selected data and image settings.

        ``resolution`` is forwarded to the training script as ``-r``. Values
        such as ``1`` and ``2`` select the original or half image resolution.
        """
        # Build the training arguments in the same order expected by train.py.
        arguments = [
            "-s", str(dataset_dir),
            "-m", str(model_dir),
            "-r", str(resolution),
        ...
        mapped = [self._container_path(item) if index in (1, 3)
                  else item for index, item in enumerate(arguments)]
        command = self._docker_command(
            self.train_image, True, ["python", "train.py"] + mapped,
        )
        self._run(command)
    ...
    def run_colmap(self, arguments):
        ...
        command = self._docker_command(
            self.colmap_image, False, ["colmap"] + mapped,
        )
        self._run(command)
