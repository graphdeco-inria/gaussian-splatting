...
def write_cameras_text(cameras, path):
    ...
    with open(path, "w") as fid:
        ...
        for _, cam in cameras.items():
            width = 436
            height = 246
            params = copy.deepcopy(cam.params)
            params = params / 2
            params[2] = width // 2
            params[3] = height // 2
            #to_write = [cam.id, cam.model, cam.width, cam.height, *cam.params]
            to_write = [cam.id, cam.model, width, height, *params]
...
if __name__ == "__main__":
    #main()
    root = '/data/jianing/dlf_result/proj_0829_all_sm'
    #ll = [os.path.join(root,i) for i in os.listdir(root) if 'colmap' in i]
    
    #for l in tqdm(ll):
    model = os.path.join(root,'sparse','0')
    cameras, images, points3D = read_model(path=model, ext='.bin')
    write_model(cameras, images, points3D, path=model, ext='.txt')
