...
def run_optimizer(NUM_ITERATIONS, kl_threshold, gaussians_gt, gaussians_init,
    ...
    with torch.no_grad():
        ...
        # sophia_losses = [[] for _ in range(num_val_cameras)]
        # sophia_images = [[] for _ in range(num_val_cameras)]

        # gaussians = gaussians_init.clone()

        # for iteration in range(NUM_ITERATIONS):
        #     rand_camera_indices = np.random.randint(0, num_cameras - 1, num_images_per_iter)
        #     viewpoint_batch = [cameras[i] for i in rand_camera_indices]
        ...
        #     loss_sophia_sum = 0.0
        #     for i in range(num_val_cameras):
        #         loss_sophia, batch_stats = loss_func(gaussians=gaussians, 
        #                                              viewpoint_cams=[primary_cams[i]],
        #                                              return_stats=True)
        #         loss_sophia_sum += loss_sophia
        #         if iteration % gif_interval == 0:
        #             sophia_losses[i].append(loss_sophia.item())
        #             sophia_images[i].append(batch_stats[0]["images"][0])
        #     print(f"Sophia loss: {loss_sophia_sum.item():.10f}")
...
def training(opt, pipe):
    ...
    O3 = np.array([0, 0, 200])
    O4 = np.array([0, 0, 300])
    viewmats1 = six_axis_view_matrices(O=O1, r=10)
    # viewmats2 = six_axis_view_matrices(O=O2, r=5)
    # viewmats3 = six_axis_view_matrices(O=O3, r=5)
    # viewmats4 = six_axis_view_matrices(O=O4, r=5)
    # viewmats = np.concatenate([viewmats1, viewmats2, viewmats3, viewmats4], axis=0)
    viewmats = viewmats1
    ...
    with torch.no_grad():
        if True:
            ...
            idx = add_gaussian(gaussians_gt,
                               xyz=torch.tensor(O1, device="cuda"),
                               scaling=torch.tensor([-0.0, -0.2, -1.2], device="cuda"),
                               rotation=torch.tensor([0.0, 1.0, 0.0, 1.0], device="cuda"),
                               ...
                               idx=0)
            # idx = add_gaussian(gaussians_gt,
            #                    xyz=torch.tensor(O1 + np.array([0.2, 0.1, 0.7]), device="cuda"),
            #                    scaling=torch.tensor([-0.9, -0.2, -0.6], device="cuda"),
            #                    rotation=torch.tensor([0.0, 0.0, 0.0, 1.0], device="cuda"),
            #                    feature_dc=torch.tensor([[0.4, 2.5, 2.1]], device="cuda"),
            #                    features_rest=0.02 * torch.ones((1, 15, 3), device="cuda"),
            #                    opacity=torch.tensor([[10.0]], device="cuda"), 
            #                    idx=idx)
            # idx = add_gaussian(gaussians_gt,
            #                    xyz=torch.tensor(O1 + np.array([1.2, 0.2, -0.4]), device="cuda"),
            #                    scaling=torch.tensor([-1.2, -0.3, -0.2], device="cuda"),
            #                    rotation=torch.tensor([0.0, 0.0, 0.8, 1.0], device="cuda"),
            #                    feature_dc=torch.tensor([[0.0, 2.5, 0.1]], device="cuda"),
            #                    features_rest=0.02 * torch.ones((1, 15, 3), device="cuda"),
            #                    opacity=torch.tensor([[10.0]], device="cuda"), 
            #                    idx=idx)
            # idx = add_gaussian(gaussians_gt,
            #                    xyz=torch.tensor(O1 + np.array([-0.6, -0.2, +1.0]), device="cuda"),
            #                    scaling=torch.tensor([-0.8, -0.1, -1.2], device="cuda"),
            #                    rotation=torch.tensor([0.4, 0.6, 0.0, 0.1], device="cuda"),
            #                    feature_dc=torch.tensor([[0.0, 0.1, 2.5]], device="cuda"),
            #                    features_rest=0.02 * torch.ones((1, 15, 3), device="cuda"),
            #                    opacity=torch.tensor([[10.0]], device="cuda"), 
            #                    idx=idx)
            # idx = add_gaussian(gaussians_gt,
            #                    xyz=torch.tensor(O4, device="cuda"),
            ...
            gaussians_init._xyz.data[0, 0] += 2.0
            gaussians_init._xyz.data[0, 2] += 0.0
            gaussians_init._xyz.data[1, 1] += 1.0
            gaussians_init._rotation.data[0] = torch.tensor([0.0, 0.5, 0.0, 0.7], device="cuda")
            gaussians_init._rotation.data[1] = torch.tensor([0.8, 0.1, 0.7, 0.1], device="cuda")
            gaussians_init._scaling.data[0, 1] -= 0.3
            gaussians_init._scaling.data[0, 2] += 0.2
            gaussians_init._xyz.data[2, :] -= 0.5
            gaussians_init._xyz.data[3, 2] -= 0.6
            gaussians_init._scaling.data[2, :] += 0.3
            gaussians_init._scaling.data[3, 0] -= 0.2
            ...
            run_optimizer(NUM_ITERATIONS, kl_threshold, gaussians_gt, gaussians_init, 
                          ...
                          loss_func, g_func, JTJv_func, Dhat_func, z_gen_func, S,
                          name=f"shortside_num-images-{num_images}")
