...
def training(opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, image_path, num_points):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        #     print("Sample:", sample)
        #     magnitude = torch.randn((1, ), device="cuda") * sigma
        #     weight = torch.exp(-0.5 * (magnitude ** 2) / (sigma ** 2))
        #     delta_x_vec = torch.randn((n,), device="cuda")
        #     delta_x_vec[g_vec == 0.0] = 0.0
        #     delta_x_vec = delta_x_vec / delta_x_vec.norm() * magnitude
        #     delta_x = GaussianModelState.zero_like_gaussians(gaussians)
        #     delta_x.load_1d_tensor(delta_x_vec, with_features_rest=False, with_exposure=False)

        #     gaussians_perturbed = deepcopy(gaussians)
        #     gaussians_perturbed.update_step(delta_x)

        #     cur_state_perturbed = LinearSolverFunctions(loss_func, gaussians_perturbed, batch_size=5, param_mask=None, damp=None, splat_mask=None, rescale=rescale)

        #     J = torch.zeros((m, n), device="cuda")
        #     J_ref = torch.zeros_like(J)
        #     g_sample, _ = cur_state_perturbed.gradient_and_loss_est(viewpoint_cams, scale=1, use_rescale=False)
        #     g_sample_vec = g_sample.as_1d_tensor(with_features_rest=False, with_exposure=False)

        #     for i in range(u_vec.shape[0]):
        #         u_vec *= 0.0
        #         u_vec[i] = 1.0
        #         u.load_1d_tensor(u_vec, with_features_rest=False, with_exposure=False)
        #         Ji = cur_state_perturbed.jvp(u, viewpoint_cams).as_1d_tensor()
        #         J[:, i] = Ji

        #     torch.cuda.synchronize()
        #     torch.save((J, g_sample_vec, magnitude, sigma), f"saved_output/sgd_picasso1/J/J_iter{iteration:05d}_sample{sample:03d}.pt")

        #     # for i in range(v_vec.shape[0]):
        #     #     print("VJP row:", i, "/", v_vec.shape[0])
        #     #     v_vec *= 0.0
        #     #     v_vec[i] = 1.0
        #     #     v.load_1d_tensor(v_vec)
        #     #     Ji = cur_state_perturbed.vjp(v, viewpoint_cams, scale=1).as_1d_tensor(with_features_rest=False, with_exposure=False)
        #     #     J_ref[i, :] = Ji

        #     # torch.cuda.synchronize()
        #     # torch.save((J_ref, magnitude, sigma), f"saved_output/sgd_picasso1/J_ref/J_ref_iter{iteration:05d}_sample{sample:03d}.pt")

        # safe_interact(local=locals(), banner="after vjp compute")
        ...
        J_smoothed = torch.zeros_like(J)
        g_smoothed_vec = torch.zeros_like(g_vec)

        import glob
        # filenames = glob.glob(f"saved_output/sgd_picasso1/J_ref/J_ref_iter{iteration:05d}_sample*.pt")
        filenames = sorted(glob.glob(f"saved_output/sgd_picasso1/J/J_iter{iteration:05d}_sample*.pt"))[:num_samples]
        total_weight = 0
        for file in filenames:
            _, _, magnitude, sigma = torch.load(file)
            weight = torch.exp(-0.5 * (magnitude ** 2) / (sigma ** 2))
            total_weight += weight
        for file in filenames:
            J_sample, g_sample_vec, magnitude, sigma = torch.load(file)
            weight = torch.exp(-0.5 * (magnitude ** 2) / (sigma ** 2))
            J_smoothed += J_sample * (weight / total_weight)
            g_smoothed_vec += g_sample_vec * (weight / total_weight)

        g_smoothed = ref_g.clone()
        g_smoothed.load_1d_tensor(g_smoothed_vec, with_features_rest=False, with_exposure=False)
        JTJ_smoothed = J_smoothed.T @ J_smoothed
        U, S, Vh = torch.linalg.svd(JTJ_smoothed)
        safe_interact(local=locals(), banner="after JTJ")

        s_gn_smoothed = -torch.linalg.solve(JTJ_smoothed + torch.eye(JTJ_smoothed.shape[0], device=JTJ_smoothed.device) * 1e-8, ref_g.as_1d_tensor(with_features_rest=False, with_exposure=False))

        s = ref_g.clone()
        s.load_1d_tensor(s_gn_smoothed, with_features_rest=False, with_exposure=False)
        s_adam = -ref_g / (ref_g.abs() + 1e-15) * rescale

        safe_interact(local=locals(), banner="after s_gn")
        ...
        with torch.no_grad():
            for i in range(-30, 100, 1):
                ...
                losses_first_order_smoothed.append(loss_0 + alpha * gv_smoothed)
        ...
        plt.plot(alphas, losses_first_order_smoothed, label="First order approx (smoothed)", alpha=0.5)
