...
def densify_gaussians(gaussians, optimizer, cap_max, opacity_prune_thresh=0.005, color_noise_lr=0.0, preserve_gaussian=False):
    ...
    if preserve_gaussian:
        gaussians.relocate_gs2(dead_mask=dead_mask, start_opacity=0.01, position_noise=0.01)
    else:
...
def run_optimizer(NUM_ITERATIONS, kl_threshold, opacity_threshold, gaussians_gt, gaussians_init,
    ...
    max_iterations = max(adam_iterations, sophia_tr_iterations)
    ...
    with torch.no_grad():
        for it in range(max_iterations):
            bg = torch.randn_like(background) * 0.1
            bgs.append(bg)
            image_gt = render(viewpoint_camera=cameras[0], pc=gaussians_gt, pipe=pipe, bg_color=bg)["render"]
            gt_images.append(image_gt)
            cameras = [build_camera(image_gt)]
            all_cameras.append(cameras)

            if it % gif_interval == 0:
                gt_images_gif.append(image_gt)
        ...
        for it in range(adam_iterations):
            loss_adam, batch_stats = loss_func(gaussians=gaussians, viewpoint_cams=all_cameras[it], return_stats=True, bg=bgs[it])
            image_adam = batch_stats[0]["images"][0]
            opacities_adam.append(gaussians.get_opacity)
            scaling_adam.append(gaussians.get_scaling.norm(dim=-1) ** 2)
            if it % gif_interval == 0:
            ...
            g = g_func(gaussians=gaussians, viewpoint_cams=all_cameras[it], bg=bgs[it])
        ...
        for it in range(sophia_tr_iterations):
            ...
            JTJv_func1 = partial(JTJv_func, gaussians=gaussians, viewpoint_cams=all_cameras[it], S=S, scale=1, bg=bgs[it])
            Dhat_func1 = partial(Dhat_func, gaussians=gaussians, viewpoint_cams=all_cameras[it], bg=bgs[it])

            loss_sophia_tr, batch_stats = loss_func(gaussians=gaussians, viewpoint_cams=all_cameras[it], return_stats=True, bg=bgs[it])
            image_sophia_tr = batch_stats[0]["images"][0]
            opacities_sophia_tr.append(gaussians.get_opacity)
            scaling_sophia_tr.append(gaussians.get_scaling.norm(dim=-1) ** 2)
            if it % gif_interval == 0:
            ...
            s_sophia_tr = clip_hellinger(gaussians, s_sophia_tr, kl_threshold, 
                                         opacity_threshold,
                                         lr)
            ...
            if with_densification and (it + 1) % densification_interval == 0 and it < 120:
                densify_gaussians(gaussians, sophia_optimizer, cap_max=cap_max, preserve_gaussian=False)
                # else:
        ...
        scaling_adam = np.array([scaling.cpu().numpy() for scaling in scaling_adam])
        scaling_sophia_tr = np.array([scaling.cpu().numpy() for scaling in scaling_sophia_tr])

        figure, axes = plt.subplots(3, 2, figsize=(6, 12))
        ...
        axes[2, 0].plot(scaling_adam)
        axes[2, 0].set_yscale("log")
        axes[2, 0].set_title("Adam Scaling Norm^2")
        axes[2, 1].plot(scaling_sophia_tr)
        axes[2, 1].set_yscale("log")
        axes[2, 1].set_title("Sophia TR Scaling Norm^2")

        plt.suptitle("Changing background")
        ...
        gif_renderer.add_series(0, 0, gt_images_gif, [0.0]*len(gt_images_gif), title="Ground Truth")
...
def training(opt, pipe):
    ...
    with torch.no_grad():
        ...
        idx = add_gaussian(gaussians_gt,
                           ...
                           feature_dc=torch.tensor([[0.4, -0.1, 0.1]], device="cuda"),
                           features_rest=0.00 * torch.ones((1, 15, 3), device="cuda"),
                           opacity=torch.tensor([[10.0]], device="cuda"), 
                           idx=0)
        idx = add_gaussian(gaussians_gt,
                           xyz=torch.tensor([-0.156, -0.7, -0.43], device="cuda"),
                           scaling=torch.tensor([-0.2, -1.2, -1.2], device="cuda"),
                           rotation=torch.tensor([0.0, 0.0, 0.0, 1.0], device="cuda"),
                           feature_dc=torch.tensor([[-0.3, 0.8, 0.1]], device="cuda"),
                           features_rest=0.00 * torch.ones((1, 15, 3), device="cuda"),
                           opacity=torch.tensor([[10.0]], device="cuda"), 
                           idx=1)
        ...
        _ = add_gaussian(gaussians_init,
                           ...
                           scaling=torch.tensor([-0.2, -1.2, -1.2], device="cuda"),
                           rotation=torch.tensor([0.0, 0.0, 0.0, 1.0], device="cuda"),
                           feature_dc=torch.tensor([[1.5, 0.1, 0.1]], device="cuda"),
                           features_rest=0.00 * torch.ones((1, 15, 3), device="cuda"),
                           opacity=torch.tensor([[1.5]], device="cuda"), 
                           idx=0)
        _ = add_gaussian(gaussians_init,
                           xyz=torch.tensor([0.36, 1.21, -0.63], device="cuda"),
                           scaling=torch.tensor([-0.5, -1.6, -1.2], device="cuda"),
                           rotation=torch.tensor([0.0, 0.0, 0.0, 1.0], device="cuda"),
                           feature_dc=torch.tensor([[0.2, 0.2, 0.2]], device="cuda"),
                           features_rest=0.00 * torch.ones((1, 15, 3), device="cuda"),
                           opacity=torch.tensor([[10]], device="cuda"), 
                           idx=1)
        _ = add_gaussian(gaussians_init,
                           xyz=torch.tensor([0.46, 1.31, -0.63], device="cuda"),
                           scaling=torch.tensor([-0.5, -1.6, -1.2], device="cuda"),
                           rotation=torch.tensor([0.0, 0.0, 0.0, 1.0], device="cuda"),
                           feature_dc=torch.tensor([[0.2, 0.2, 0.2]], device="cuda"),
                           features_rest=0.00 * torch.ones((1, 15, 3), device="cuda"),
                           ...
                           idx=1)
        _ = add_gaussian(gaussians_init,
                           xyz=torch.tensor([0.56, 1.11, -0.63], device="cuda"),
                           scaling=torch.tensor([-0.5, -1.6, -1.2], device="cuda"),
                           rotation=torch.tensor([0.0, 0.0, 0.0, 1.0], device="cuda"),
                           feature_dc=torch.tensor([[0.2, 0.2, 0.2]], device="cuda"),
                           features_rest=0.00 * torch.ones((1, 15, 3), device="cuda"),
        ...
        run_optimizer(NUM_ITERATIONS, kl_threshold, opacity_threshold, gaussians_gt, gaussians_init, 
                      pipe, cameras, background, lr,
                      loss_func, g_func, JTJv_func, Dhat_func, z_gen_func, S,
                      gif_renderer, name="changing_background", with_densification=True,
