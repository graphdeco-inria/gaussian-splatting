...
from solver.function_smoother import smoothed_func
...
class SmoothedJvp:
    def __init__(self, gaussians, partial_state):
        self.gaussians = gaussians
        self.partial_state = partial_state

    def __call__(self, u, viewpoint_cams):
        class Inner:
            def __init__(self, u, viewpoint_cams, partial_state):
                self.u = u
                self.viewpoint_cams = viewpoint_cams
                self.partial_state = partial_state
            def __call__(self, gaussians):
                cur_state = self.partial_state(gaussians=gaussians)
                return cur_state.jvp(self.u, self.viewpoint_cams)
        return smoothed_func(Inner(u, viewpoint_cams, self.partial_state), self.gaussians)
...
class SmoothedJTJv:
    ...
    def __call__(self, u, viewpoint_cams, scale):
        class Inner:
            def __init__(self, u, viewpoint_cams, scale, partial_state):
                self.u = u
                self.viewpoint_cams = viewpoint_cams
                self.scale = scale
                self.partial_state = partial_state
            def __call__(self, gaussians):
                cur_state = self.partial_state(gaussians=gaussians)
                return cur_state.JTJv(self.u, self.viewpoint_cams, self.scale)
        return smoothed_func(Inner(u, viewpoint_cams, scale, self.partial_state), self.gaussians)
...
def training(opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, image_path, num_points, all_columns):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        partial_state = partial(LinearSolverFunctions, loss_func=loss_func, batch_size=5, param_mask=None, damp=None, splat_mask=None, rescale=rescale)

        smoothed_jvp_func = SmoothedJvp(gaussians, partial_state)
        smoothed_jtjv_func = SmoothedJTJv(gaussians, partial_state)
        ...
        v = smoothed_jvp_func(u, viewpoint_cams)
        ...
        dirname = "figures/smoothed_jvp_column_sum"
        ...
        for i in cols_list:
            ...
            Ji = smoothed_jvp_func(u, viewpoint_cams).as_1d_tensor()
            JTJu = smoothed_jtjv_func(u, viewpoint_cams, scale=1).as_1d_tensor(with_features_rest=False, with_exposure=False)
            uJTJu = (u_vec * JTJu).sum().item()
            loss_vec0 = loss_func(gaussians=gaussians, viewpoint_cams=viewpoint_cams).as_1d_tensor()
            loss_scalar0 = loss_vec0.norm() ** 2
            ...
            if g_vec[i] != 0.0:
                with torch.no_grad():
                    ...
                    plt.plot(alphas, predicted_deltas1, label="Predicted delta", alpha=0.5)
                    ...
                    plt.savefig(f"{dirname}/smoothed_jvp_column_sum_col{i:04d}.png")
                    plt.close('all')
