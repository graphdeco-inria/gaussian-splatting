...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, gif_interval):
    ...
    """
    1. Pick one frame to debug
    2. Render it. Use opacity gradient to find the most contributing gaussians
    3. Keep only those gaussians for training
    """

    # DEBUG
    print("Reducing viewpoints")
    debug_camera_idx = 182
    viewpoint_stack = [viewpoint_stack[debug_camera_idx]]
    ...
    render_args = {"iteration": first_iter,
                   "opt": opt,
                   "pipe": pipe,
                   "bg": bg,
    ...
    loss_init = loss_func(gaussians=gaussians, viewpoint_cams=viewpoint_stack)
    g_init = g_func(gaussians=gaussians, viewpoint_cams=viewpoint_stack)

    active_gidx = g_init.opacity.flatten().abs().argsort(descending=True)[:num_points]

    gaussians_copy = gaussians.clone()
    gaussians._xyz.data = gaussians_copy._xyz.data[active_gidx]
    gaussians._features_dc.data = gaussians_copy._features_dc.data[active_gidx]
    gaussians._features_rest.data = gaussians_copy._features_rest.data[active_gidx]
    gaussians._scaling.data = gaussians_copy._scaling.data[active_gidx]
    gaussians._rotation.data = gaussians_copy._rotation.data[active_gidx]
    gaussians._opacity.data = gaussians_copy._opacity.data[active_gidx]
    gaussians._exposure.data = gaussians_copy._exposure.data[debug_camera_idx]

    # # Problematic gaussian for this view
    # print("Removing problematic gaussian")
    # gaussians._opacity.data[287] -= 1000

    loss_reduced_init = loss_func(gaussians=gaussians, viewpoint_cams=viewpoint_stack)
    print(f"Initial loss before pruning: {loss_init.item():.6f}, after pruning: {loss_reduced_init.item():.6f}")
    safe_interact(local=locals(), banner="After pruning gaussians prompt")
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            active_gidx = g.opacity.nonzero(as_tuple=False).squeeze(-1)
            print(f"Number of active gaussians: {active_gidx.shape[0]} / {gaussians._xyz.shape[0]}")

            # TODO: Fix this. Using L1 loss to get correct batch stats
            if opt.loss_type != "l1" and opt.naive_densification:
                _, _, batch_stats = g_func(gaussians=gaussians, viewpoint_cams=viewpoint_batch, return_stats=True, loss_type="l1")
            ...
            for i, vc in enumerate(viewpoint_stack):
                ...
                if iteration % gif_interval == 0 and i < len(sophia_images):
            ...
            #     sophia_losses.append(loss.item())
            ...
            if False:
                ...
                for i in range(n):
                    # Skip features_rest
                    if i >= N1 + N2 and i < N1 + N2 + N3:
                        continue

                    print(f"Computing JTJ column {i+1}/{n} at iteration {iteration}", end='\r')
                    H[:, i] = compute_JTJ_col(JTJv_func1, i, gaussians)

                D = H.diagonal()
                sorted_indices = torch.argsort(D, descending=True)
                D_est = sophia_optimizer.D_est.as_1d_tensor()

                figure, ax = plt.subplots(figsize=(8, 6))
                plt.plot(range(n), D[sorted_indices].cpu().numpy(), label='Diagonal of JTJ')
                plt.plot(range(n), D_est[sorted_indices].cpu().numpy(), label='Estimated Diagonal (D_hat)')
                ax.set_xlim(0, 4500)
                plt.yscale('log')
                plt.legend()

                plt.savefig(f"figures/debug_train_diagonal_iteration_{iteration}.png")

            # safe_interact(local={**locals(), **globals()}, banner=f"Debug prompt at iteration {iteration}")
    ...
    gif_renderer = GifRenderer(num_rows=1, num_cols=2, figsize=(10, 6), gif_interval=1)
    gif_renderer.add_gt(0, 0, viewpoint_stack[0].original_image)
    name = "Sophia TR (Ours)" if not opt.use_adam else "Adam"
    gif_renderer.add_series(0, 1, sophia_images[0], sophia_losses[0], title=name)
    fname = f"figures/train_debug_sophia_tr.gif" if not opt.use_adam else f"figures/train_debug_adam.gif"
