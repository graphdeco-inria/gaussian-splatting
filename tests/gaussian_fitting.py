...
def run_optimizer(NUM_ITERATIONS, kl_threshold, gaussians_gt, gaussians_init,
    ...
    with torch.no_grad():
        ...
        for _ in range(NUM_ITERATIONS):
            ...
            loss_sophia, batch_stats = loss_func(gaussians=gaussians, viewpoint_cams=cameras, return_stats=True)
    ...
    gif_renderer.add_series(1, 1, sophia_tr_images, sophia_tr_losses, title="Sophia TR")

    gif_renderer.animate(f"gaussian_fitting_{name}.gif", interval=500)
    print(f"Saved gaussian_fitting_{name}.gif")
...
def training(opt, pipe):
    ...
    with torch.no_grad():
        if False:
            ...
            gaussians_init._xyz.data[0, 1] += 0.3
