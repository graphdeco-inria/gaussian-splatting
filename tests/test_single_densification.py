...
def run_optimizer(NUM_ITERATIONS, kl_threshold, opacity_threshold, gaussians_gt, gaussians_init,
    ...
    with torch.no_grad():
        ...
        for it in range(sophia_tr_iterations):
            ...
            s_sophia_tr = clip_hellinger(gaussians, s_sophia_tr, kl_threshold, opacity_threshold, lr)
        ...
        plt.suptitle("Densification from Single Gaussian No Noise")

        plt.tight_layout()
        plt.savefig("figures/single_densification_no-noise.png")
...
def training(opt, pipe):
    ...
    with torch.no_grad():
        ...
        idx = add_gaussian(gaussians_gt,
                           ...
                           idx=0)
        idx = add_gaussian(gaussians_gt,
                           xyz=torch.tensor([-0.156, -0.6, -0.43], device="cuda"),
                           scaling=torch.tensor([-0.2, -1.2, -1.2], device="cuda"),
                           rotation=torch.tensor([0.0, 0.0, 0.0, 1.0], device="cuda"),
        ...
        run_optimizer(NUM_ITERATIONS, kl_threshold, opacity_threshold, gaussians_gt, gaussians_init, 
                      pipe, cameras, background, lr,
                      loss_func, g_func, JTJv_func, Dhat_func, z_gen_func, S,
                      gif_renderer, name="single_densification", with_densification=True,
