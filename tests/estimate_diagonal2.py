...
from solver.preconditioner import AdaptiveAdaHessianPreconditioner
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    S = GaussianModelState.ones_like_gaussians(gaussians)
    S.xyz_grad *= rescale.xyz_scale
    S.features_dc_grad *= rescale.features_dc_scale
    S.features_rest_grad *= rescale.features_rest_scale
    S.scaling_grad *= rescale.scaling_scale
    S.rotation_grad *= rescale.rotation_scale
    S.opacity_grad *= rescale.opacity_scale
    S.exposure_grad *= rescale.exposure_scale

    rademacher_gen = partial(GaussianModelState.rademacher_like_gaussians, gaussians)
    preconditioner = AdaptiveAdaHessianPreconditioner(rademacher_gen, beta2=0.999, eps=1e-16, hessian_power=1.0, S=S, update_S=False)

    iters = [10, 20, 50, 100, 200, 500, 1000, 2000]
    # iters = [10]
    Ds = {}

    preconditioner.reset()
    preconditioner.update(JTJx, warmup_cam_provider, warmup_scale, num_iter = max(iters),
                          debug_iters=iters)
