...
from gaussian_renderer.single_render import single_render
from solver.batch_training_loss import batch_training_loss
from solver.single_training_loss import single_training_loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, gif_interval):
    ...
    viewpoint_stack_baseline = scene.getTrainCameras().copy()
    ...
    batch_render_pkg = batch_render([viewpoint_stack[0]], gaussians, pipe, background, use_trained_exp=train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE, track_weights=False)
    single_render_pkg = single_render([viewpoint_stack[0]], gaussians, pipe, background, use_trained_exp=train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE, track_weights=False)

    batch_loss = batch_training_loss(gaussians=gaussians, viewpoint_cams=[viewpoint_stack[0]], 
                                     return_stats=False,
                                     track_weights=False, 
                                     **render_args)
    single_loss = single_training_loss(gaussians=gaussians, viewpoint_cams=[viewpoint_stack[0]], 
                                     return_stats=False,
                                     track_weights=False, 
                                     **render_args)

    batch_loss_val = 0.5 * (batch_loss.norm() ** 2)
    single_loss_val = 0.5 * (single_loss.norm() ** 2)

    gaussians.zero_grad()
    batch_loss_val.backward()
    batch_grad = GaussianModelVector.from_gaussians_grad(gaussians)

    gaussians.zero_grad()
    single_loss_val.backward()
    single_grad = GaussianModelVector.from_gaussians_grad(gaussians)
    ...
    with torch.enable_grad(), fwAD.dual_level(), gaussians.make_dual(v):
        viewpoint_cams_batch = [viewpoint_cam]
        batch_loss_dual = batch_training_loss(gaussians=gaussians, viewpoint_cams=viewpoint_cams_batch, **render_args)
        batch_loss_primal, batch_loss_tangent = fwAD.unpack_dual(batch_loss_dual)

    with torch.enable_grad(), fwAD.dual_level(), gaussians.make_dual(v):
        viewpoint_cams_batch = [viewpoint_cam]
        single_loss_dual = batch_training_loss(gaussians=gaussians, viewpoint_cams=viewpoint_cams_batch, **render_args)
        single_loss_primal, single_loss_tangent = fwAD.unpack_dual(single_loss_dual)

    safe_interact(local=locals(), banner="Debug prompt before training loop")
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # if not viewpoint_stack_baseline:
        #     viewpoint_stack_baseline = scene.getTrainCameras().copy()
        #     viewpoint_cam = viewpoint_stack_baseline.pop(randint(0, len(viewpoint_stack_baseline)-1))
        # else:
        #     viewpoint_cam = viewpoint_stack_baseline.pop(randint(0, len(viewpoint_stack_baseline)-1))
        rand_idx = np.random.choice(viewpoint_indices, 1, replace=False)
        viewpoint_cam = viewpoint_stack_baseline[rand_idx[0]]
        ...
        render_time_baseline = iter_start.elapsed_time(render_end)
        backward_time_baseline = render_end.elapsed_time(iter_end)

        print(f"ITER {iteration} render time (baseline): {render_time_baseline} ms, backward time (baseline): {backward_time_baseline} ms")
        ...
        with torch.no_grad():
            ...
            viewpoint_batch = []
            print("rand indices:", rand_indices, len(viewpoint_stack))
            ...
            # if iteration % 200 == 1:
            if False and iteration == 21000:
                safe_interact(local=locals(), banner="Debug prompt at iteration 21000")
            ...
            print(f"ITER {iteration} gradient time: {gradient_time} ms")
