...
def training(opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, image_path, num_points, all_columns, sum_column):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        H_rescaled = S_mat @ H @ S_mat
        Sg = S_mat @ g_vec

        H_rescaled = H_rescaled + torch.eye(n, device=H_rescaled.device) * damp

        s_gn_ref = torch.linalg.solve(H_rescaled + torch.eye(n, device=H_rescaled.device) * 1e-15, -Sg)
        ...
        dirname = f"figures/newton_column_sum_rescale_xyz-{xyz_scale}_dc-{features_dc_scale}_rest-{featuress_rest_scale}_scaling-{scaling_scale}_rotation-{rotation_scale}_opacity-{opacity_scale}/"
        os.system(f"rm -rf {dirname}")
        os.makedirs(f"{dirname}", exist_ok=False)
        ...
        for i in cols_list:
            ...
            if g_vec[i] != 0.0:
                with torch.no_grad():
                    ...
                    plt.xlabel("Alpha")
                    plt.ylabel("Delta")
                    plt.title(f"HVP Column {i} sum check")
                    plt.ylim(-1e-3, 1e-3)

                    plt.axvline(x=s_gn_ref[i].item(), color='r', linestyle='--', label='Newton step')

                    plt.legend()


                    plt.savefig(f"{dirname}/newton_column_sum_col{i:04d}.png")
                    plt.close('all')
