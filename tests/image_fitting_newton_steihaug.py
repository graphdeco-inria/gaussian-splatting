...
def training(opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, image_path, num_points, all_columns, sum_column):
    ...
    xyz_scale = xyz_scale * torch.ones_like(gaussians._xyz)
    features_dc_scale = features_dc_scale * torch.ones_like(gaussians._features_dc)
    features_rest_scale = featuress_rest_scale * torch.ones_like(gaussians._features_rest)
    scaling_scale = scaling_scale * torch.ones_like(gaussians._scaling)
    rotation_scale = rotation_scale * torch.ones_like(gaussians._rotation)
    opacity_scale = opacity_scale * torch.ones_like(gaussians._opacity)
    exposure_scale = exposure_scale * torch.ones_like(gaussians._exposure)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            loss_before = 0.0
            for vc in viewpoint_cams:
                loss_before += reference_training_loss(iteration, opt, vc, gaussians, pipe, bg, train_test_exp=train_test_exp, depth_l1_weight=depth_l1_weight)

        # gaussians.update_step(best_alpha * v)
        print("Updating gaussians with s_newton")
        gaussians.update_step(s_newton)

        with torch.no_grad():
            loss_after = 0.0
            for vc in viewpoint_cams:
                loss_after += reference_training_loss(iteration, opt, vc, gaussians, pipe, bg, train_test_exp=train_test_exp, depth_l1_weight=depth_l1_weight)

        adjust_param_thresh = 0.01
        if loss_after.item() > loss_before.item():
            update_rescale = 0.99
        else:
            update_rescale = 1.01

        y = y.as_1d_tensor(with_exposure=False, with_features_rest=False)
        y = y / y.abs().max()
        large_indices_mask = y.abs() > adjust_param_thresh
        rescale_vec = rescale.as_1d_tensor(with_exposure=False, with_features_rest=False)
        rescale_vec[large_indices_mask] *= update_rescale
        rescale_vec = rescale_vec.clamp(min=1e-6, max=1e0)
        rescale.load_1d_tensor(rescale_vec, with_exposure=False, with_features_rest=False)
        print(f"adjusted scale for indices {large_indices_mask.nonzero(as_tuple=False).squeeze().cpu().numpy()}")
