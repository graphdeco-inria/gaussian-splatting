...
def training(opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, image_path, num_points, all_columns, sum_column):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        s_gn_ref = torch.linalg.solve(JTJ_ref + torch.eye(JTJ_ref.shape[0], device=JTJ_ref.device) * 1e-15, -Sg)
        ...
        dirname = f"figures/gn_column_sum_rescale_xyz-{xyz_scale}_dc-{features_dc_scale}_rest-{featuress_rest_scale}_scaling-{scaling_scale}_rotation-{rotation_scale}_opacity-{opacity_scale}/"
