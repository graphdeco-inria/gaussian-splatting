...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, jvp_start, num_images):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if use_first_order:
            ...
            best_loss_pcg = solver_functions.evaluate_loss(val_cameras, val_scale)[0]
            ...
            while True:
                ...
                print(f"[ITER {iteration}] alpha {cur_alpha_pcg:.2f}, loss {loss_scalar:.6f}")

                if loss_scalar < best_loss_pcg:
                    best_loss_pcg = loss_scalar
                    best_alpha_pcg = cur_alpha_pcg
                    increase_count_pcg = 0
                if loss_scalar > best_loss_pcg:
                    increase_count_pcg += 1
            ...
            # Line search on the sgd direction
            alpha_sgd = 0.0
            cur_alpha_sgd = 0.0
            best_alpha_sgd = 0.0
            val_scale = len(val_cameras) / len(val_cameras)
            best_loss_sgd = solver_functions.evaluate_loss(val_cameras, val_scale)[0]
            print(f"[ITER {iteration}] SGD Line search start: alpha {cur_alpha_sgd:.2f}, loss {best_loss_sgd:.6f}")
            increase_count_sgd = 0
            while True:
                gaussians.update_step(s_sgd * (alpha_sgd - cur_alpha_sgd))
                cur_alpha_sgd = alpha_sgd
                loss_scalar = solver_functions.evaluate_loss(val_cameras, val_scale)[0]
                print(f"[ITER {iteration}] alpha {cur_alpha_sgd:.2f}, loss {loss_scalar:.6f}")
                if loss_scalar < best_loss_sgd:
                    best_loss_sgd = loss_scalar
                    best_alpha_sgd = cur_alpha_sgd
                    increase_count_sgd = 0
                if loss_scalar > best_loss_sgd:
                    increase_count_sgd += 1
                if increase_count_sgd >= 5:
                    break
                if alpha_sgd == 0.0:
                    alpha_sgd += 0.1
                else:
