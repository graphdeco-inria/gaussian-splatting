...
**To unlock the full frame rate, please disable V-Sync on your machine and also in the application (Menu &rarr; Display). In a multi-GPU system (e.g., laptop) your OpenGL/Display GPU should be the same as your default CUDA GPU (or the one whose CUDA index you pass with --device) for maximum performance.**
