        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration % opt.opacity_reset_interval == 0 or (white_background and iteration == opt.densify_from_iter):
                    opacity_new = gaussians.inverse_opacity_activation(
                            torch.min(gaussians.get_opacity, 
                                      torch.ones_like(gaussians.get_opacity)*0.01))
                    gaussians._opacity.data = opacity_new
                    adam_optimizer.reset_opacity()
                    sophia_optimizer.reset()
                    # safe_interact(local=locals(), banner=f"After opacity reset at iteration {iteration} prompt")

            if (iteration in checkpoint_iterations):
                print("\n[ITER {}] Saving Checkpoint".format(iteration))
                print("Model path: {}".format(args.model_path + "/chkpnt" + str(iteration) + ".pth"))
